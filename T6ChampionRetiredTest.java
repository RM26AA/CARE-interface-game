...
public class T6ChampionRetiredTest {
    ...
    @Test
    public void retireChampionMultipleTimes() {
        String championName = "Neon";
        int expected = 0;
        int actual = game.retireChampion(championName);
        assertEquals(expected, actual); //First retirement should succeed

        expected = 2;
        actual = game.retireChampion(championName); //Second retirement should fail
        assertEquals(expected, actual);
    }

    @Test
    public void retireChampionTeamEmpty() {
        // Retire all champions from the team
        game.retireChampion("Ganfrank");
        game.retireChampion("Elblond");
        game.retireChampion("Neon");

        int expected = 2;
        int actual = game.retireChampion("Flimsi"); // Attempting to retire when team is empty
        assertEquals(expected, actual);
    }
